...
import com.example.tqtruongcnpmct2.databinding.ItemOrderBinding;
import com.example.tqtruongcnpmct2.model.Order;
import com.example.tqtruongcnpmct2.utils.DateTimeUtils;
...
public class OrderAdapter extends RecyclerView.Adapter<OrderAdapter.OrderViewHolder> {
    ...
    public void onBindViewHolder(@NonNull OrderViewHolder holder, int position) {
        ...
        if(order.getStatust().equals("Đang Giao")){
            holder.mItemOrderBinding.tvStatus.setText(R.string.danggiao);
            holder.mItemOrderBinding.tvStatus.setTextColor(Color.BLUE);
        }
        if(order.getStatust().equals("Đã Hủy")){
            holder.mItemOrderBinding.tvStatus.setText(R.string.dahuy);
            holder.mItemOrderBinding.tvStatus.setTextColor(Color.RED);
        }
        if(order.getStatust().equals("Đã Giao")){
            holder.mItemOrderBinding.tvStatus.setText(R.string.dagiao);
            holder.mItemOrderBinding.tvStatus.setTextColor(Color.GREEN);
        }
        if(order.getStatust().equals("Chờ Duyệt")){
            holder.mItemOrderBinding.tvStatus.setText(R.string.choduyet);
            holder.mItemOrderBinding.tvStatus.setTextColor(Color.BLACK);
        }
